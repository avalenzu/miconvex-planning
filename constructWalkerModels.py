...
legLength = 0.5
r0 = [0, legLength/2]
rf = [3*legLength, legLength]
v0 = [0, 0]
w0 = 0
hipOffset = {'left_toe': {'x': 0.0, 'z': -0.25}, 'right_toe': {'x': 0.0, 'z': -0.25},
             'left_heel': {'x': 0.0, 'z': -0.25}, 'right_heel': {'x': 0.0, 'z': -0.25}}
step_height = legLength/2
platform1_start = -1*legLength
platform1_end = 6*legLength
...
hop.addFreeBlock(bottom=platform1_height/legLength, right=platform1_end/legLength)
...
def _legLengthRule(m, side, t):
    heel = '%s_heel' % side
    return m.p[heel, 'x', t]**2 + m.p[heel, 'z', t]**2 <= 1
m_nlp.legLengthConstraint = Constraint(['left', 'right'], m_nlp.t, rule=_legLengthRule)

def _footLengthRule(m, side, t):
    toe = '%s_toe' % side
    heel = '%s_heel' % side
    return (m.foot[toe, 'x', t]**2 - 2*m.foot[toe, 'x', t]*m.foot[heel , 'x', t] + m.foot[heel , 'x', t]**2 +
            m.foot[toe, 'z', t]**2 - 2*m.foot[toe, 'z', t]*m.foot[heel , 'z', t] + m.foot[heel , 'z', t]**2) == 0.1**2
m_nlp.footLengthConstraint = Constraint(['left', 'right'], m_nlp.t, rule=_footLengthRule)

def _footOrientationRule(m, side, t):
    toe = '%s_toe' % side
    heel = '%s_heel' % side
    return m.foot[toe, 'x', t] >= m.foot[heel , 'x', t]

m_nlp.footOrientationConstraint = Constraint(['left', 'right'], m_nlp.t, rule=_footOrientationRule)
